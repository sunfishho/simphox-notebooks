...
class MNISTDataProcessor:
    def __init__(self, fashion: bool=False):
        ...
        print(f"raw train shape: {self.x_train_raw.shape}")
        self.x_train_ft = np.fft.fftshift(np.fft.fft2(self.x_train_raw), axes=(1, 2))
        print(f"ft train shape: {self.x_train_ft.shape}")
    ...
    def fourier(self, freq_radius):
        ...
        print(f"after flattening: {norm_inputs(x_train_ft.reshape((self.num_train, -1))).astype(np.complex64).shape}")
        return ONNData(
